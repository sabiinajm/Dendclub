...
import ClinicForm from "./components/Register/ClinicForm";
...
const Register = () => {
  ...
  const handleSubmit = () => {
    if (!formData) return alert("Zəhmət olmasa məlumatları doldurun.");

    localStorage.setItem("userData", JSON.stringify(formData));
    if (activeTab === "doctor") {
      navigate("/Həkim-Dashboard");
    } else if (activeTab === "clinic") {
      navigate("/Klinika-Dashboard");
    } else {
      navigate("/Ana-səhifə");
    }
  ...
  const renderForm = () => {
    switch (activeTab) {
      case "user":
        return <UserForm navigate={navigate} onChange={handleFormChange} />;
      case "doctor":
        return <DoctorForm navigate={navigate} onChange={handleFormChange} />;
      case "clinic":
        return <ClinicForm navigate={navigate} onChange={handleFormChange} />;
      default:
        return null;
    }
  };
    ...
    <div className="max-w-[1400px] mx-auto ">
      ...
      <div className="flex items-center justify-center flex-col absolute top-0 left-0 z-50 bg-white w-full">
        <div className="max-w-[640px] pt-[100px]">
          ...
          <div>{renderForm()}</div>
          <p className="mt-12 text-[#636366] text-[15px] font-light px-4 sm:p-0">
            Qeydiyyatdan keçməklə siz bizim
            <div>
              <span className="text-[#0D9CD8] underline cursor-pointer">
                {" "}
        ...
        <div className="flex flex-col items-center justify-center mt-12 ">
          <button onClick={handleSubmit} className="font-medium text-base text-white bg-[#0D9CD8] w-[300px] sm:w-[528px] h-[52px] rounded-lg mb-5 ">
            Qeydiyyatdan keç
          </button>
          <div className="flex gap-[33px] mb-[33px] items-center">
            <div className="sm:w-[203px] h-[1px] bg-[#D1D1D6]"></div>
            <p className="text-base font-normal text-[#2C2C2E]">Və ya</p>
            <div className="sm:w-[203px] h-[1px] bg-[#D1D1D6]"></div>
          </div>
          <div className="w-[300px] sm:w-[528px] h-[52px] border border-[#D7D7D7] rounded-[14px] flex items-center justify-center gap-[10px] cursor-pointer mb-3">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
          ...
          <div className="w-[300px] sm:w-[528px] h-[52px] border border-[#D7D7D7] rounded-[14px] flex items-center justify-center gap-[10px] cursor-pointer">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
          ...
          <p className="mt-[50px] pb-7 text-base text-[#000000] font-normal ">Artıq hesabınız var? <Link to={'/Login'} className="text-[#0D9CD8] underline cursor-pointer">Daxil olun</Link></p>
        </div>
      </div>
    </div>
  );
