...
//                 <h1 className="pt-[3rem] text-[#030303] text-2xl md:text-[2.5rem] leading-[56px] text-center">Apteklərimiz</h1>
...
//                         <Link to={`/Apteklərimiz/${index + 1}`} key={index} className="min-h-[427px] relative bg-white shadow-lg rounded-[20px] overflow-hidden">
//                             <img className="min-h-[323px] w-full object-cover" src="/assets/images/aptek.png" alt={`Aptek ${index + 1}`} />
//                             <p className="text-start py-7 pl-3 text-[1.5rem] md:text-[1.5rem] font-medium text-[#2C2C2E]">Kanon Aptek</p>
...
const fakePharmacies = [...Array(9)].map((_, i) => ({
    id: i + 1,
    name: `Kanon Aptek ${i + 1}`
}));
...
function Pharmacy() {
    ...
    const filteredPharmacies = useMemo(() => {
        ...
        return fakePharmacies.filter(
            pharmacy => startsWith(pharmacy.name)
        );
    }, [searchTerm]);
    ...
    return (
        <main className="bg-[#f2f5f8] pt-[24px]">
            <div className="max-w-[1224px] mx-auto px-2">
                <h1 className="pt-[3rem] text-[#030303] text-2xl md:text-[2.5rem] leading-[56px] text-center">
                    Apteklərimiz
                </h1>
                <div className="min-h-[150px] flex items-center">
                    <div className="flex flex-col md:flex-row gap-4 items-center">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-12 justify-center">
                    {filteredPharmacies.map((pharmacy, index) => (
                        <Link
                            to={`/Apteklərimiz/${pharmacy.id}`}
                            key={pharmacy.id}
                            className="min-h-[427px] relative bg-white shadow-lg rounded-[20px] overflow-hidden"
                        >
                            <img
                                className="min-h-[323px] w-full object-cover"
                                src="/assets/images/aptek.png"
                                alt={pharmacy.name}
                            />
                            <p className="text-start py-7 pl-3 text-[1.5rem] md:text-[1.5rem] font-medium text-[#2C2C2E]">
                                {pharmacy.name}
                            </p>
                            <div className="h-[56px] w-[56px] flex justify-center items-center absolute bottom-[20px] right-[20px] rounded-full bg-[#E1F6FF]">
                                <svg xmlns="http://www.w3.org/2000/svg" width="29" height="30" viewBox="0 0 29 30" fill="none">
                ...
                <p className="text-[#2C2C2E] text-2xl font-medium text-center py-[80px] min-h-[350px]">
                    Daha çox
                </p>
            </div>
        </main>
