...
function Header() {
  ...
  return (
    <header className="min-h-[131px] bg-white">
      <div className="min-h-[112px] max-w-[1224px] mx-auto py-4 flex justify-between items-center ">
        <img
          className="h-[112px] w-[240px] object-cover"
          src="/assets/images/logo.png"
          alt="Medclub Logo"
        />
        <nav
          className={`justify-center gap-6 lg:justify-between flex-col lg:flex-row fixed lg:static top-0 right-0 bg-[#fff] z-50 h-screen lg:h-auto shadow-lg lg:shadow-none items-center list-none flex overflow-hidden min-w-[320px] lg:w-[677px] ${
            showMenu ? "translate-x-0" : "translate-x-[400px] lg:translate-x-0"
          } transition-all duration-400`}
        >
          <IoCloseOutline
            onClick={toggleMenu}
            className="absolute top-[55px] right-[10px] text-4xl hover:scale-125 transition-all cursor-pointer duration-160 hover:text-[#0D9CD8] lg:hidden"
          />
          <NavLink
            to="/"
            className={({ isActive }) =>
              isActive
                ? "border-b-2 border-[#0D9CD8] whitespace-nowrap"
                : "whitespace-nowrap"
            }
          >
            Ana səhifə
        ...
        <Link
          to={"Çat"}
          className="rounded-full absolute xl:static top-[100px] right-[10px] flex justify-center xl:w-[145px] p-[8px] bg-[#0D9CD8] text-white xl:rounded-[12px]"
        >
        ...
        <div className="flex items-center min-w-[121px] gap-3 justify-between">
          ...
          <Link
            to={"Profil"}
            className="h-[48px] w-[48px] flex cursor-pointer justify-center items-center bg-[#F2F5F8] rounded-full"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
          ...
          <div
            onClick={toggleMenu}
            className="px-2 text-3xl cursor-pointer lg:hidden"
          >
